...
check_versions() {
    ...
    if check_command rustc; then
        ...
        check_command cargo && log_success "Cargo version: $(cargo --version)"
    elif [ -x /opt/rust/bin/rustc ]; then
        log_success "Rust version: $(/opt/rust/bin/rustc --version)"
        log_success "Cargo version: $(/opt/rust/bin/cargo --version)"
    else
    ...
    # Neon CLI (neonctl) — サーバーレス PostgreSQL
    if check_command neonctl; then
        log_success "Neon CLI (neonctl) version: $(neonctl --version)"
    else
        log_warning "Neon CLI (neonctl): Not installed"
    fi
    ...
    if check_command doppler; then
        log_success "Doppler version: $(doppler --version)"
    else
        log_warning "Doppler: Not installed"
    fi

    #==============================
    # ☁️ クラウド (GCP — メインクラウド)
    #==============================
    log_header "☁️ クラウド (GCP)"
    if check_command gcloud; then
        log_success "Google Cloud CLI version: $(gcloud --version | head -n1)"
    elif [ -x /opt/google-cloud-sdk/bin/gcloud ]; then
        log_success "Google Cloud CLI version: $(/opt/google-cloud-sdk/bin/gcloud --version | head -n1)"
    else
        log_warning "Google Cloud CLI (gcloud): Not installed"
    fi
